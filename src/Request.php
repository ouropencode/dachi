...
class Request
{
    ...
    protected static $uri_variables = [];
    protected static $arguments = [];
    protected static $api_mode = false;

    protected static $render_path = '';

    protected static $output_data = [];
    protected static $response_code = ['status' => 'assumed', 'message' => 'Assuming successful.'];
    ...
    public static function getFullUri()
    {
        ...
        if (self::$uri === [] && isset($_GET['dachi_uri'])) {
            self::$uri = explode('/', trim($_GET['dachi_uri'], " \t\n\r\0\x0B/"));
        }
     ...
     * @param string $key    The key to set
     * @param string $value  The value to set
     * @param int    $expire The time to expire (defaults to -1, -1 is 'now + 30 days')
    ...
    {
        self::$response_code = ['status' => $status, 'message' => $message];

...
class DocumentCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (file_exists($DS(['vendor', 'ld-packages', 'dachi']))) {
            $response = shell_exec($DS(['vendor', 'bin', 'apigen']).' generate -s src -s '.$DS(['vendor', 'ld-packages', 'dachi', 'src']).' -d '.$destination.' '.$flags.' --title Dachi');
        } else {
            $response = shell_exec($DS(['vendor', 'bin', 'apigen']).' generate -s src -d '.$destination.' '.$flags.' --title Dachi');
        }
